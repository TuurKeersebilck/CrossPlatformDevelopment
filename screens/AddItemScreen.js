...
const AddItemScreen = ({ navigation }) => {
	...
	const handleAdd = async () => {
		...
		if (type !== "artist" && !selectedArtist)
			newErrors.selectedArtist = "Artist is required";
		if (type === "album" && !releaseDate)
			newErrors.releaseDate = "Release date is required";
		if (type === "track" && !duration)
		...
		try {
			...
			Alert.alert(
				`${type} Added`,
				`The ${type.toLowerCase()} "${name}" has been added successfully.`
			);
		} catch (error) {
		...
		return (
			<View>
				...
				<TouchableOpacity
					...
					accessibilityLabel="Select an artist"
				>
		...
		return (
			<View>
				...
				<TouchableOpacity
					...
					accessibilityRole="button"
					accessibilityLabel="Select an album"
				>
			...
			<ScrollView contentContainerStyle={{ flexGrow: 1 }}>
				...
				>
					...
					<View style={styles.buttonContainer}>
						<Button
							title="Artist"
							onPress={() => handleTypeChange("artist")}
							accessibilityRole="button"
							accessibilityLabel="Select artist type"
						/>
						<Button
							title="Album"
							onPress={() => handleTypeChange("album")}
							accessibilityRole="button"
							accessibilityLabel="Select album type"
						/>
						<Button
							title="Track"
							onPress={() => handleTypeChange("track")}
							accessibilityRole="button"
							accessibilityLabel="Select track type"
						/>
					</View>
					...
					{type !== "" && (
						<>
							...
							<TextInput
								...
								placeholderTextColor={colors.secondaryText}
								accessibilityLabel="Enter image URL"
							/>
								...
								<>
									...
									{errors.bio && (
										<Text
											...
											accessibilityLabel={`Error: ${errors.bio}`}
										>
											{errors.bio}
										</Text>
									)}
							...
							{type === "album" && (
								<>
									...
									{errors.releaseDate && (
										<Text
											style={[styles.errorText, { color: colors.errorText }]}
											accessibilityLabel={`Error: ${errors.releaseDate}`}
										>
											{errors.releaseDate}
										</Text>
									)}
									<TextInput
										...
										placeholderTextColor={colors.secondaryText}
										accessibilityLabel="Enter release date (YYYY-MM-DD)"
									/>
								</>
							)}
							...
							{type === "track" && (
								<>
									...
									{errors.duration && (
										<Text
											style={[styles.errorText, { color: colors.errorText }]}
											accessibilityLabel={`Error: ${errors.duration}`}
										>
											{errors.duration}
										</Text>
									)}
									<TextInput
										...
										placeholderTextColor={colors.secondaryText}
										accessibilityLabel="Enter duration (e.g., 03:45)"
									/>
								</>
							)}
